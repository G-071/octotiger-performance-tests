...
kernel_args="--executors_per_gpu=128 --max_gpu_executor_queue_length=1024 --hydro_device_kernel_type=CUDA --hydro_host_kernel_type=DEVICE_ONLY --amr_boundary_kernel_type=AMR_OPTIMIZED --max_kernels_fused=1 --number_gpus=1 "
# Scenario
...
for cores in ${CoreList}; do
  for executors in $ExecutorList; do
    if (( ${executors} == 0 )) ; then
      ...
      kernel_args=" --hpx:threads=${cores} --executors_per_gpu=${executors} --max_gpu_executor_queue_length=1024 --hydro_device_kernel_type=OFF --hydro_host_kernel_type=LEGACY --amr_boundary_kernel_type=AMR_OPTIMIZED --max_kernels_fused=1 --number_gpus=1 "
      ...
      kernel_args=" --hpx:threads=${cores} --executors_per_gpu=${executors} --max_gpu_executors_queue_length=1024 --hydro_device_kernel_type=CUDA --hydro_host_kernel_type=DEVICE_ONLY --amr_boundary_kernel_type=AMR_OPTIMIZED --max_kernels_fused=${slices} --number_gpus=1 "
        output1="$(./build/octotiger/build/octotiger ${hpx_args} ${kernel_args} ${octotiger_args} )"
        echo "DEBUG: ${output1}" >> ${debug_log_filename}
        # cleanup
